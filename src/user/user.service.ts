...
export class UserService {
    ...
    async saveQueueDataForUser(userId: number): Promise<void> {
        ...
        console.log(
            `Found ${activities.length} swapped activities for userId=${userId}`,
        );
        ...
        for (const activity of activities) {
            if (
                activity.tokens &&
                Array.isArray(activity.tokens) &&
                activity.tokens.length === 2
            ) {
                const [fromToken, toToken] = activity.tokens;

                const fromAmountUsd = parseFloat(
                    fromToken['amountUsd'][0]?.replace(',', '') || '0',
                );
                const toAmountUsd = parseFloat(
                    toToken['amountUsd'][0]?.replace(',', '') || '0',
                );

                const profit = toAmountUsd - fromAmountUsd;
                const profitPercentage =
                    fromAmountUsd > 0 ? (profit / fromAmountUsd) * 100 : 0;
        ...
        await Promise.all(
            validActivities.map(
                async ({ activity, profit, profitPercentage }) => {
                    ...
                    if (existingQueueEntry) {
                        console.log(
                            `Queue entry already exists for activityId: ${activity.id}. Skipping.`,
                        );
                        return;
                    }
                    ...
                    newQueueEntry.fromTokenChainId =
                        fromToken['chainId'] || null;
                    ...
                    newQueueEntry.fromTokenAmount = parseFloat(
                        fromToken['amount'][0]?.replace(',', '') || '0',
                    );
                    newQueueEntry.fromTokenAmountUsd = fromAmountUsd;
                    newQueueEntry.fromTokenIsPositive =
                        fromToken['isPositive'] || null;
                    ...
                    newQueueEntry.toTokenAmount = parseFloat(
                        toToken['amount'][0]?.replace(',', '') || '0',
                    );
                    newQueueEntry.toTokenAmountUsd = toAmountUsd;
                    newQueueEntry.toTokenIsPositive =
                        toToken['isPositive'] || null;
                    ...
                    try {
                        ...
                        console.log(
                            `Queue entry saved for activityId: ${activity.id}`,
                        );
                    } catch (error) {
                        if (error.code === '23505') {
                            console.log(
                                `Duplicate entry for activityId: ${activity.id}. Skipping.`,
                            );
                        } else {
                            console.error(
                                `Error saving queue entry for activityId: ${activity.id}:`,
                                error.message,
                            );
                        }
                    }
